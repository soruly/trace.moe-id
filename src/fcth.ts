...
export const FCTH: FeatureExtractor = {
  ...
  extract(image: PixelData): FeatureResult {
    ...
    for (let i = 0; i < packedLen; i++) {
      const high = quantized[i << 1] * 2;
      const low = (i << 1) + 1 < 192 ? quantized[(i << 1) + 1] * 2 : 0;
  ...
  distance(a: number[] | Uint8Array, b: number[] | Uint8Array): number {
    ...
    if (a instanceof Uint8Array || a.length <= 96) {
      histA = new Array(192).fill(0);
      for (let i = 0; i < a.length; i++) {
        const tmp = (a[i] + 128) & 0xff;
        histA[i << 1] = ((tmp >> 4) & 0x0f) / 2.0;
        if ((i << 1) + 1 < 192) histA[(i << 1) + 1] = (tmp & 0x0f) / 2.0;
      }
    } else {
      histA = a as number[];
    }

    if (b instanceof Uint8Array || b.length <= 96) {
      histB = new Array(192).fill(0);
      for (let i = 0; i < b.length; i++) {
        const tmp = (b[i] + 128) & 0xff;
        histB[i << 1] = ((tmp >> 4) & 0x0f) / 2.0;
        if ((i << 1) + 1 < 192) histB[(i << 1) + 1] = (tmp & 0x0f) / 2.0;
      }
    } else {
      histB = b as number[];
    }
